    ...
    this._Init = function (initCallback) {
        if (!_initialized) {
            ...
            if (this._setSelected != null && this._setSelected != "") {
                this.SelectNode(this.GetNodeFromDataPath(this._setSelected), false);
            }
...
EPi.TreeView.prototype.NavigateToNode = function (itemDataPath, expandSelectedBranch) {
    ...
    var listNode = this.GetListNode(itemDataPath);
...
EPi.TreeView.prototype.RefreshChildren = function (itemDataPath) {
    var node = this.GetNodeFromDataPath(itemDataPath);
    //TODO: Figure out what to do when we can't find the node 
    if (node) {
...
EPi.TreeView.prototype.RefreshParent = function (itemDataPath) {
    var node = this.GetNodeFromDataPath(itemDataPath);
    if (node != null) {
...
EPi.TreeView.prototype.RefreshChildrenIfVisible = function (itemDataPath) {
    var node = this.GetNodeFromDataPath(itemDataPath);
...
EPi.TreeView.prototype.GetNodeFromDataPath = function (itemDataPath) {
    return this.GetListNode(itemDataPath);
}
...
EPi.TreeView.prototype.GetDataPath = function (listNode) {
    ...
    return listNode.id.substr(this._treeContainer.id.length + 1);
}

EPi.TreeView.prototype.GetListNode = function (dataPath) {
    return document.getElementById(this._treeContainer.id + "_" + dataPath);
}
...
EPi.TreeView.prototype.PopulateNode = function (listNode) {
    ...
    this.DoCallback(listNode, "populate", dataPath);
}
...
EPi.TreeView.prototype.DoCallback = function (contextNode, nodeState, dataPath) {
    var contextObject = { populationNode: contextNode, treeViewObject: this };
    var treeObject = this;
...
EPi.TreeView.prototype.HighLightNode = function (dataPath) {
    var node = this.GetListNode(dataPath);
    if (node) {

...
$.widget("ui.daterange", {
    ...
    _doRedirect: function () {
        ...
        param[this.options.queryStartDateKey] = $.datepicker.formatDate(this.options.queryDateFormat, this.options.startDate);
        param[this.options.queryEndDateKey] = $.datepicker.formatDate(this.options.queryDateFormat, this.options.endDate);
        var query = '?';
        for (var key in param) {
            query += (query != '?' ? '&' : '') + key + '=' + param[key];
        }
        document.location.href = location.pathname + query;
    },
    ...
    _init: function () {
        ...
        $('<button type="button">' + 'Select' + '</button>').appendTo(btnArea).addClass('ui-daterange-button').bind('click', function (e) {
            e.preventDefault();
            _this.select();
        });
        $('<button type="button">' + 'Cancel' + '</button>').appendTo(btnArea).addClass('ui-daterange-button').bind('click', function (e) {
            e.preventDefault();
            _this.cancel();
        });
    ...
    select: function () {
        if (this.options.startDate <= this.options.endDate) {
            if (this.options.startDate != this._previousStartDate || this.options.endDate != this._previousEndDate) {
                this._doRedirect();
            }
            this.hide();
            if (typeof this.options.onSelect == 'function') {
                this.options.onSelect.call(this.element);
            }
        }
        else {
